...
void vTaskMotor(void *pvParameters)
{
    ...
    // Setare Timer2 în Fast PWM cu frecvență fixă
    TCCR2A = (1 << WGM21) | (1 << WGM20) | (1 << COM2B1);  
    TCCR2B = (1 << WGM22) | (1 << CS21);  // Prescaler 8

    OCR2A = PWM_DUTY_BUFF;  // Frecvență ~14kHz
    OCR2B = (int)(PWM_DUTY_BUFF * pwmDutyCycle);   // Duty cycle ~50%
...
void vTaskEncoder(void *pvParameters)
{
    for (;;)
    {
        ...
        case TRAN_PWM_DUTY:
            ...
            OCR2B = (int)(PWM_DUTY_BUFF * pwmDutyCycle);   // Duty cycle
            break;
        case SENSOR_VALUES:
            break;
